...
def node_runtime_gate(plugin: str) -> None:
    ...
    js = f'''\nconst EXEC_STATE_LOCATE = "locate"\nconst EXEC_STATE_MUTATE = "mutate"\nconst EXEC_STATE_REPAIR = "repair"\nconst EXECUTE_REPLACE_NODE_TOOL = "execute_replace_node"\nconst EXECUTE_RENAME_SYMBOL_TOOL = "execute_rename_symbol"\nconst SCOUT_RENAME_TARGET_PROTOCOL = "scout-rename-target-v2"\nconst MUTATION_TOOL_NAMES = Object.freeze([EXECUTE_REPLACE_NODE_TOOL, EXECUTE_RENAME_SYMBOL_TOOL])\n{shape}\n{frontier}\n\nfunction assert(cond, msg) {{ if (!cond) throw new Error(msg) }}\n\nassert(validateMutationShape({{kind:"replace_node",before:"x",replacement:"y"}}).ok === true, "replace valid")\nassert(validateMutationShape({{kind:"replace_node",before:"x",replacement:""}}).ok === true, "deletion valid")\nassert(validateMutationShape({{kind:"replace_node"}}).ok !== true, "replace missing fields")\nassert(validateMutationShape({{kind:"rename_symbol",new_name:"z"}}).ok === true, "rename valid")\nassert(validateMutationShape({{kind:"rename_symbol"}}).ok !== true, "rename missing name")\n\nconst local = {{\n  executionState: EXEC_STATE_MUTATE,\n  localMutationCapability: {{replaceNodeReady:true, renameSymbolReady:false, globalReady:false}},\n  localMutationCandidates: [{{target:{{}}}}],\n  renameMutationCapability: null,\n  scoutHandoffPath: "handoff.json",\n  mutationIntent: "generic_edit",\n}}\nassert(JSON.stringify(allowedToolsForState(local)) === JSON.stringify([EXECUTE_REPLACE_NODE_TOOL]), "local generic frontier")\n\nconst global = {{\n  ...local,\n  localMutationCapability: {{replaceNodeReady:true, renameSymbolReady:false, globalReady:true}},\n  renameMutationCapability: {{\n    protocol: SCOUT_RENAME_TARGET_PROTOCOL,\n    ready: true,\n    globalReady: true,\n    operation: "rename_symbol",\n    sourceHandoffPath: "handoff.json",\n    target: {{file:"a.py",symbol_name:"foo"}},\n  }},\n}}\nconst globalUnknown = {{...global, mutationIntent:"unknown"}}\nconst globalGeneric = {{...global, mutationIntent:"generic_edit"}}\nconst globalRename = {{...global, mutationIntent:"rename_symbol"}}\nassert(JSON.stringify(allowedToolsForState(globalUnknown)) === JSON.stringify([]), "unknown intent fails closed")\nassert(JSON.stringify(allowedToolsForState(globalGeneric)) === JSON.stringify([EXECUTE_REPLACE_NODE_TOOL]), "global generic frontier")\nassert(JSON.stringify(allowedToolsForState(globalRename)) === JSON.stringify([EXECUTE_RENAME_SYMBOL_TOOL]), "global rename frontier")\nassert(allowedToolsForState(globalUnknown).length <= 1, "unknown frontier cardinality")\nassert(allowedToolsForState(globalGeneric).length <= 1, "generic frontier cardinality")\nassert(allowedToolsForState(globalRename).length <= 1, "rename frontier cardinality")\n\nconst noCandidates = {{...globalRename, localMutationCandidates:[]}}\nassert(JSON.stringify(allowedToolsForState(noCandidates)) === JSON.stringify([EXECUTE_RENAME_SYMBOL_TOOL]), "rename independent of replace candidates")\nconst noRenameProof = {{...globalGeneric, renameMutationCapability:null}}\nassert(JSON.stringify(allowedToolsForState(noRenameProof)) === JSON.stringify([EXECUTE_REPLACE_NODE_TOOL]), "generic replace independent of rename proof")\nconst renameNoProof = {{...globalRename, renameMutationCapability:null}}\nassert(JSON.stringify(allowedToolsForState(renameNoProof)) === JSON.stringify([]), "rename requires rename proof")\nconst stickyRename = {{...globalGeneric, executionState:EXEC_STATE_REPAIR, activeMutationTool:EXECUTE_RENAME_SYMBOL_TOOL}}\nassert(JSON.stringify(allowedToolsForState(stickyRename)) === JSON.stringify([EXECUTE_RENAME_SYMBOL_TOOL]), "repair action sticky")\nconsole.log("PASS action-specific schema semantics + intent-capability frontier runtime")\n'''
...
def main() -> None:
    ...
    # Rename authority is intentionally independent from the replace owner.
    ...
    assert 'renameCapability?.ready === true' in frontier
    assert 'renameCapability?.globalReady === true' in frontier
