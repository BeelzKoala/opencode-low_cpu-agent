...
try {
  ...
  const decoratedKinds = await compilePythonAdditiveEdits({
    root,
    target,
    declarations: [
      "@cache\ndef cached():\n    return 1\n\n" +
    ...
    resolveSite: async ({ content }) => {
      ...
      return {
        ...
        before,
        replacement: `${content}${before}`,
        site_sha256: "4".repeat(64),
      }
    },
  })
  assert.equal(decoratedKinds.ok, true)
  assert.equal(decoratedKinds.ir_normalization.declarations_retained, 3)
  assert.match(decoratedKinds.edits[0].replacement, /^@cache\n/u)
  assert.match(
    decoratedKinds.edits[0].replacement,
    /@transactional\(\)\nasync def/u,
  )
  assert.match(
    decoratedKinds.edits[0].replacement,
  ...
  const reclassifiedDeclaration = await compilePythonAdditiveEdits({
    root,
    target,
    ...
    resolveSite: async ({ content }) => {
      ...
      return {
        ...
        before,
        replacement: `${content}${before}`,
        site_sha256: "3".repeat(64),
      }
    },
  })
  assert.equal(reclassifiedDeclaration.ok, true)
  assert.equal(
    reclassifiedDeclaration.ir_canonicalizer_protocol,
    "python-ir-canonicalizer-v1",
  )
  assert.equal(
    reclassifiedDeclaration.ir_normalization.imports_reclassified,
    1,
  )
  assert.equal(
    reclassifiedDeclaration.ir_normalization.declarations_retained,
    1,
  )
  assert.equal(reclassifiedDeclaration.edits.length, 2)
  assert.equal(
    reclassifiedDeclaration.edits[0].compiler_operation,
    "add_imports",
  )
  assert.equal(
    reclassifiedDeclaration.edits[1].compiler_operation,
  ...
  const unsafeStatement = await compilePythonAdditiveEdits({
    root,
    target,
    declarations: ["x = dangerous_call()"],
    maxReplacementBytes: 12 * 1024,
    resolveSite: async () => {
      throw new Error("unsafe statement must fail before site resolution")
    },
  })
  assert.equal(unsafeStatement.ok, false)
  assert.equal(
    unsafeStatement.reason,
    "python_declaration_statement_kind_invalid",
  )

  const toolingDirective = await compilePythonAdditiveEdits({
    root,
    target,
    declarations: ["import io  # noqa: F401"],
    maxReplacementBytes: 12 * 1024,
  })
  assert.equal(toolingDirective.ok, false)
  assert.equal(
    toolingDirective.reason,
    "python_fragment_tooling_directive_unsupported",
  )
} finally {
...
console.log(
  "PASS E2.5/R2.1 current typed Python compiler contract: AST canonicalization + decorator preservation + virtual syntax preflight",
)
